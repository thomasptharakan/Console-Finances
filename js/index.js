...
for (i = 0; i < finances.length; i++) {
    ...
    if ((i + 1) < finances.length){
        // Track what the total change in profits are from month to month
        changeProfit = finances[i+1][1] - finances[i][1];
        // The net total amount of Profit/Losses over the entire period.
        netChangeProfit = netChangeProfit + changeProfit;
        if (greatestIncrease < changeProfit){
            // The greatest increase in profits (date and amount) over the entire period.
            greatestIncreaseMonth = finances[i+1][0];
            greatestIncrease = changeProfit;
        }
        if (greatestDecrease > changeProfit){
            // The greatest decrease in losses (date and amount) over the entire period.
            greatestDecrease = changeProfit;
